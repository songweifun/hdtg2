...
//        }
    ...
    private function parseGlobalConst($content)
    {
        ...
        $preg = '/\$Hd.(get|post|request|cookie|Session)\./is';
        //$replace = '\'\$_\'.strtoupper("\1").".";';
        return preg_replace_callback($preg, function($matches){

            return '\'\$_\'.strtoupper($matches[0]).".";';
...
//        $preg = '/{\s*(\$[^=!<>\)\(\+\;]+)}/ieU'; //以{$或$开头的进行解析处理
        ...
        */
        ...
        $this->content = preg_replace_callback($preg, function($matches){
            return '<?php echo '. $this->parseVar($matches[0]).';?>';
        }, $this->content);
    }
